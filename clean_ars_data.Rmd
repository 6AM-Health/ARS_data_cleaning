...
long_df <- ars_temp4 %>% gather(product_id, product_name, product_1:product_16)
long_df1 <- long_df %>% select(-price_1:-price_16) %>% filter(product_name!='NA')

#converting prices 
long_df2 <- ars_temp4 %>% gather(price_id, unit_price_bd, price_1:price_16)
long_df2 <- long_df2 %>% select(-product_1:-product_16) %>% filter(unit_price_bd!='NA')
...
```{r}
write.csv(df2, '~/Downloads/6AMHealth/cleaned_arsdata.csv')
```
